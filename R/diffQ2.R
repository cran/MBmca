"diffQ2" <- function(xy, fct = max, fws = 8, col = 2, plot = FALSE, verbose = FALSE, 
			 ...
			 inder = FALSE)
{
	    ...
	    TmD1	<-	diffQ(xy, fct = fct, fws = fws, negderiv = negderiv, verbose = TRUE, rsm = rsm, inder = inder)

	    if (TmD1[["temperature"]][["mean.T.delta"]] >= 0.5) {

			tmp.xy <- 	predict(smooth.spline(TmD1$xy[, 1], TmD1$xy[, 2]), 
					    seq(min(TmD1$xy[, 1]), max(TmD1$xy[, 1]), TmD1[["temperature"]][["mean.T.delta"]] / 1.3))
			xy.smoothed <- 	data.frame(tmp.xy$x, tmp.xy$y)
		} else (xy.smoothed <-  data.frame(TmD1$xy[, 1], smooth.spline(TmD1$xy[, 1], TmD1$xy[, 2])$y))

	    # Calculates the second derivative and from the first derivative for the minimum of the melting curve.
	    Tm1D2	<-	diffQ(xy.smoothed, fct = min, fws = fws, verbose =  TRUE, col = col, peak = peak, 
						   deriv = deriv, negderiv = FALSE, derivlimits = derivlimits, 
						   derivlimitsline = derivlimitsline, vertiline = vertiline,
						   inder = inder)
	    # Calculates the second derivative and from the first derivative for the maximum of the melting curve.
	    Tm2D2	<-	diffQ(xy.smoothed, fct = max, fws = fws, verbose =  TRUE, col = col, peak = peak, 
						   deriv = deriv, negderiv = FALSE, derivlimits = derivlimits, 
						   derivlimitsline = derivlimitsline, vertiline = vertiline,
						   inder = inder)
	    ...
	    # Plot the first derivative
		  ...
		  plot(TmD1$xy, xlab = "Temperature", ylab = "-d(F) / dT", type = "b")
		    ...
		    if (derivlimits) {points(TmD1$limits.xQ, TmD1$limits.diffQ, cex = 1, pch = 19, col = col)}

	    # Plot the second derivative
		  par(fig = c(0,1,0,0.525), new = TRUE)
		  plot(Tm1D2$xy, xlim = c(min(TmD1$xy[, 1]), max(TmD1$xy[, 1])), xlab = "Temperature", ylab = "-d^2(F) / dT^2", type = "b")
		...
		} else (return(list(Tm = TmD1$Tm, fluoTm = TmD1$fluoTm, xTm1.2.D2 = x, yTm1.2.D2 = y)))
