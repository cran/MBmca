...
{ 
	  ...
	  temperature <- list(T.delta = deltaT, mean.T.delta = deltaT.mean, sd.T.delta = deltaT.sd, RSD.T.delta = deltaT.RSD)
	  ...
	  for (i in 2:9) {
	    ...
	    if (class(try(na.omit(xQ[(RANGE - i):(RANGE + i)]), silent = T)) == "try-error") {
	      ...
	      lm2sum <- summary(lm2)
	      list.res[[i-1]] <- list(i, limits.xQ, limits.diffQ, fluo.x, lm2, coeflm2, coeflm2.y, lm2sum)
	    }
	  }
	  ...
	  if ((!plot) && (.Device != "null device")) {
	    ...
	    if (derivlimits) {points(limits.xQ, limits.diffQ, cex = 1, pch = 19, col = col)}
	    if (derivlimitsline) {lines(spline(limits.xQ[1:length(limits.xQ)], lm2[["fitted.values"]][1:length(lm2[["fitted.values"]])]), col = "orange", lwd = 2)}
	  ...
	  {		RMSE <- sqrt(mean(residuals(model)^2))
			...
			if(NRMSE > 0.08) {NRMSE.warning <- "NRMSE bad"
					  } else (NRMSE.warning <- "NRMSE ok")
			return(list(NRMSE = NRMSE, RMSE = RMSE, NRMSE.warning = NRMSE.warning))
	  }
	  ...
	  if (verbose) {
			return(list(Tm = abl, fluoTm = y, Tm.approx = dev[1], fluo.x = fluo.x, xy = out, limits.xQ = limits.xQ, limits.diffQ = limits.diffQ,
				    adj.r.squared = lm2sum$adj.r.squared, NRMSE = NRMSE.res$NRMSE, fws = list.res$fw, devsum=dev.sum, temperature = temperature, fit = summary(list.res$lm2)))
